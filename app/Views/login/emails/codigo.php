...
/** @var string $code */
$email = $email ?? '';
$codigo = $code ?? 'Houve um erro ao processar o seu pedido.';
if($codigo === 'Houve um erro ao processar o seu pedido.'){
    $backgound = '#ff0000';
}else{
    $backgound = '#5840ff';
}
...
<div style="max-width: 600px; margin: 40px auto; background-color: #ffffff; box-shadow: 0px 4px 10px rgba(0, 0, 0, 0.1); border-radius: 8px; padding: 20px;">
    ...
    <p style="font-size: 16px; color: #444444; margin: 10px 0; font-weight: 600;">
        ...
        Use o código abaixo para validar seu email:
    </p>

    <!-- Bloco do código -->
    <div style="background-color: #f0f8ff; color: <?= $backgound ?>; font-size: 20px; font-weight: 900; padding: 15px; border-radius: 5px; display: inline-block; margin: 20px 0; letter-spacing: 3px;">
       <?= $codigo ?>
    </div>
